...
let gesamt = [];
// Gesamtlänge / 100 Zeichen pro Abschnitt = 27 Abschnitte mit 100 Zeichen
let anzahlTexte = Math.ceil(text.length / 100);

// Zwei Zählvariablen die man später in der Schleife auch noch erhöht.
let anfangSub = 0;
let endeSub = 100;
const trennen = () => {
    //Schleife von 1 - Anzahl der Texte (27)
    for (i = 1; i <= anzahlTexte; i += 1) {
        //Füge dieser Variablen 100 Zeichen hinzu. 
        let newText = text.substring(anfangSub, endeSub);

        //Am Ende von jedem newText noch einen String anhängen
        newText += `- ${i} of gesamte-Nachrichten`;

        //Zählervariablen erhöhen um 100 Schritte
        anfangSub += 100;
        endeSub += 100;

        //Die 100 Zeichen + den angehängten String ins Array speichern
        gesamt.push(newText);
        console.log(newText);
    }
